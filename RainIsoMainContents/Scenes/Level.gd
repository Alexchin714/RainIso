...
func _ready():
	var map_with_visible_notifier = load("res://RainIsoMainContents/Scenes/TileMap.tscn")
	#var second_map_with_visible_notifier = load("res://RainIsoMainContents/Scenes/TileMapSemiForeground.tscn")
	var map_size = 1
	var chunk_size = 1
	...
	for chunk_x in range(map_size / chunk_size):
		...
		#	var map_d = second_map_with_visible_notifier.instance()
		...
		#	map_d.chunk_rect.position = Vector2(chunk_size * chunk_x, chunk_size * chunk_y)
		#	map_d.chunk_rect.size = Vector2(chunk_size, chunk_size)
			for x in range(chunk_size):
				for y in range(chunk_size):
					map_c.set_cell(map_c.chunk_rect.position.x + x, map_c.chunk_rect.position.y + y, chunk_x + chunk_y)
				#	map_d.set_cell(map_d.chunk_rect.position.x + x, map_d.chunk_rect.position.y + y, chunk_x + chunk_y)
			add_child(map_c)
		#	add_child(map_d)
...
func _on_OthersideArea_body_entered(body):
	$OthersideSymbolNode/AnimationPlayer.play("SymbolFader")

func _on_MelodyTownHeart_body_entered(body):
	$Completion/MelodyTownLevelComplete/AnimationPlayer.play("bonuslevelcomplete")
