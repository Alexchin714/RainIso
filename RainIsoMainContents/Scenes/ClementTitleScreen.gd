...
func _physics_process(delta):
	...
	if state == ACTIVE:
		...
		var down = $Skeleton/Body/LedgeDown
		var forward = $Skeleton/Body/LedgeFoward
		...
		if $Skeleton/Body/LedgeDown.is_colliding() and $Skeleton/Body/LedgeDown.get_collision_normal().y >= 0.99:
		
			var right = $Skeleton/Body/LedgeFoward.get_collision_normal().cross(Vector3(0, 1, 0))
	...
	if state == HANGING:
		$Skeleton/Body/LedgeFoward.force_raycast_update()
		$Skeleton/Body/LedgeDown.force_raycast_update()
		if not $Skeleton/Body/LedgeFoward.is_colliding() or $Skeleton/Body/LedgeFoward.get_collider() != hangingFrom:
			translation = prevPos
