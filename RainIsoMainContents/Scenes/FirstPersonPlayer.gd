...
func _physics_process(delta):
	...
	if Input.is_action_pressed("3dW"):
		direction.z += -1
	if Input.is_action_pressed("3dS"):
		direction.z += 1
	if Input.is_action_pressed("3dA"):
		direction.x += -1
	if Input.is_action_pressed("3dD"):
		direction.x += 1
	...
	if is_on_floor():
		if Input.is_action_pressed("shift") or Input.get_joy_axis(0, 6) >= 0.6:
	...
	if Input.is_action_just_pressed("undo") or Input.is_joy_button_pressed(0, JOY_XBOX_B):
