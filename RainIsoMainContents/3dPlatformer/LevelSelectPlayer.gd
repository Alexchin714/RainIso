...
func _physics_process(delta):
	...
	if state == ACTIVE:
		...
		if $Skeleton/ClementModel/LedgeDown.is_colliding() and $Skeleton/ClementModel/LedgeDown.get_collision_normal().y >= 0.99:
		
			var right = $Skeleton/ClementModel/LedgeForward.get_collision_normal().cross(Vector3(0, 1, 0))
	...
	if state == HANGING:
		$Skeleton/ClementModel/LedgeForward.force_raycast_update()
		$Skeleton/ClementModel/LedgeDown.force_raycast_update()
		if not $Skeleton/ClementModel/LedgeForward.is_colliding() or $Skeleton/ClementModel/LedgeForward.get_collider() != hangingFrom:
			translation = prevPos
