...
func _on_FreezingPoint_body_entered(body):
	if body.name == "Player":
		body.isFreezing = true
		body.isHeating = false
	$FreezeScreen/AnimationPlayer.play("Freeze")

func _on_FreezingPoint_body_exited(body):
	if body.name == "Player":
		body.isFreezing = false
		body.isHeating = true
	$FreezeScreen/AnimationPlayer.play_backwards("Freeze")
